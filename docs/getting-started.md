...
若重新发布后在 Smart Life 的场景候选设备中仍完全看不到网关，连接真机调试并查看
`Panel scene eligibility diagnostics` 控制台日志。依次排除共享设备、非家庭管理员账号、
设备实例云端 DP 数少于本地 DP 数。三项都正常仍不可见时，记录 Device ID、PID、产品
版本和云端 DP 数，提交涂鸦工单检查产品实例的场景联动索引。不要通过修改 KNX 组地址
或 MQTT 配置处理这个问题，它们不参与 Smart Life 场景候选设备的生成。
